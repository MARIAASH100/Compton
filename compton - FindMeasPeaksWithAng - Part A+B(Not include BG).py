...
class InteractivePeakSelector:
    def __init__(self, x, y):
        ...
        self.ax.plot(x, y, label="Raw Signal")
    ...
    for i, (x0, x1) in enumerate(regions):
        ...
        for model_func, label in [(gauss_linear, 'Gauss+Linear'), (gauss_exp, 'Gauss+Exp')]:
            try:
                ...
                # Plotting the fit with and without background
                plt.figure()
                plt.plot(x_seg, y_seg, 'b.', label='Data')
                plt.plot(x_seg, model_func(x_seg, *popt), 'orange', label='Fit: Gaussian + Background')
                plt.plot(x_seg, gaussian(x_seg, *popt[:3]), 'red', label='Gaussian only')
                plt.xlabel("Channel")
                plt.ylabel("Counts")
                plt.title(f"Angle {angle}, Peak {i+1}, Fit: {label}")
                plt.legend()
                plt.show()
...
def main():
    ...
    for fname in sorted(os.listdir(folder)):
        if fname.endswith('.mca'):
            ...
            corrected = counts  # no background subtraction (unlike in the previous .py)
